...
- Use `config/local.env` for local automation settings. This project already includes your recipient number there.
...
4. Send or archive the digest, including iMessage if you want the top intel on your phone.
...
The built-in phone-text path now uses the macOS Messages app through [scripts/imessage.sh](</Users/tobiloba202/Documents/New project/scripts/imessage.sh>).

1. Make sure Messages is signed in to iMessage on this Mac.
2. Confirm your recipient number in [config/local.env](</Users/tobiloba202/Documents/New project/config/local.env>).
...
```bash
./scripts/imessage.sh --recipient +16614258372 --file reports/latest_sms_digest.txt
```
...
```bash
/Users/tobiloba202/Documents/New\ project/scripts/run_daily.sh
```
