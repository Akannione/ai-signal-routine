...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--send-digests",
        action="store_true",
        help="Send digests if Slack, SMTP, or local iMessage script settings are available.",
    )
    ...
    if not args.skip_digests:
        ...
        email_path, slack_path, sms_path = write_digests(email_digest, slack_digest, sms_digest, outdir)
...
def _send_digests_if_configured(email_digest: str, slack_digest: str, sms_digest: str) -> None:
